...
pub fn display_br(aircraft_raw: String) -> Result<(), JsValue> {
    ...
    let elem = document
        .get_element_by_id("your_br")
        .ok_or(JsValue::from_str("Cant find element your_br"))?;
    ...
    if let Some(highest_br) = &sorted_by_br.get(0) {
        let highest = document
            .get_element_by_id("highest_br")
            .ok_or(JsValue::from_str("Cant find element highest_br"))?;
