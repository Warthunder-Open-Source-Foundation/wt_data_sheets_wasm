...
pub fn make_shell_options() -> Result<(), JsValue> {
    ...
    for y in ShellType::iter() {
        ...
        document
            .get_element_by_id("select_ammo_type")
            .unwrap()
            .append_child(&elem)
            .unwrap();
    }
    ...
    for shell in SHELLS.iter() {
        if selected == shell.shell_type.to_string() {
            ...
            if &shell.caliber != &shell.true_caliber {
                let _caliber_cell_filled = caliber_cell
                    .set_inner_html(&format!("{} ({})", &shell.caliber, &shell.true_caliber));
            } else {
            ...
            let _penetration_cell_filled = penetration_cell
                .set_inner_html(&shell.penetration.first().unwrap_or(&(0, 0)).1.to_string());

            let explosive_cell = document.create_element("td").unwrap();
            let _explosive_cell_filled = explosive_cell.set_inner_html(
                &(match &shell.explosive {
                    ExplosiveType::Inert => 0,
                    ExplosiveType::Energetic(explosive) => explosive.equiv_mass,
                })
                .to_string(),
            );
