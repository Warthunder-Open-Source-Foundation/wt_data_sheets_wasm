...
pub fn generate_tank_list() -> Result<(), JsValue> {
    ...
    document
        .get_element_by_id("column_1")
        .unwrap()
        .set_inner_html("Gunner/Scope");
    document
        .get_element_by_id("column_2")
        .unwrap()
        .set_inner_html("Commander");

    let mut i = 0;
    for thermal in THERMALS.iter().enumerate() {
        let row = document.create_element("tr").unwrap();
        row.set_attribute("id", &thermal.1.name).unwrap();
        row.set_attribute("class", &format!("{} {:?}", &i, &thermal.1.vehicle_type))
            .unwrap();
